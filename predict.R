...
#setwd("E:/datascientist/NccaPrediction")
...
for (i in 1:nrow(NCAAF_L1)) {
  if(i %% 2 != 0){
    ...
    ## Load current ELO into the main NCAAF_L1 dataset ##
...
win_prob_glm_1 <- glm(Result ~ Elo_Difference + Home, family = binomial, data = NCAAF_L1 %>% filter(Season>2013, Season <= 2018))
...
NCAAF_This_Week <- NCAAF_L1_Future %>% filter(Date> "2020-12-30",Date<= "2021-01-02") %>% 
...
names(NCAAF_This_Week) <- c("Date","Season","Team","Opponent","Home","Neutral_Location",
                            "Game_ID","ELO","Opp_ELO")
